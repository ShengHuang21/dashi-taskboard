...
1. Search for an existing issue before creating one. Use `context current`, then list the project issues and compare their identifiers, titles, descriptions, and status.
   ...
   - For the Owner-facing `My tasks` view, verify the originating Owner request and label its exact existing issue `owner-goal`, preserving other labels and relations. An Agent-created issue or a nested issue can represent an Owner request; creator type and root position do not establish that provenance. Keep a stable, short title in the Owner's language and record source evidence in its description or comments. Do not pass this label to internal subtasks merely because Agents decomposed the work, and do not ask the Owner to classify those subtasks. The view shows each explicitly marked task's own progress across all descendant levels, including archived work; unsplit unfinished tasks remain unestimated. This label classifies presentation only and grants no execution or delivery authority.
