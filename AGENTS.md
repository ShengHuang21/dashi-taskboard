...
- Owner-approved exception (2026-09-18, this Taskboard repository only): Task Model Routing V1 / CAP-72 and its goal-team adapter may dispatch future bounded roles using Sheng's `personal-codex-skills/docs/model-routing-policy.md` risk tiers, mapped light/standard/deep to simple+fast/standard+balanced/complex+capable. Preserve any explicit task/role model pin; resolve and validate the actual model ID and reasoning effort against the host catalog before dispatch, and report unavailable profiles rather than silently substituting. Technical validators retain distinct actual identities and the candidate's required risk tier and effort. This exception changes only role-profile selection, not independent review, tests, authority, global defaults, or already-running conversations; outside this routing path the same-model rule above remains in force. Skills/MD owns the policy and this exception; Taskboard owns the adapter and scheduling, and must not claim tiered execution until actual dispatch consumes the profile.
...
- For complex or risky work, use an independent Codex reviewer by default. The reviewer must not have authored the candidate and must be able to read the complete changed code and the relevant surrounding source, requirements, and verification evidence. Give it the exact repository, base and head revisions, and the complete candidate diff; a PR URL or summary alone is not sufficient.
- The reviewer returns a complete written verdict bound to those revisions (and the tree or diff digest for an uncommitted candidate), including the scope actually read, actionable correctness findings, and any access or verification gaps. Incomplete code access or an unfinished response is not approval. Refresh affected review evidence when the candidate or integration context changes.
- ChatGPT web Pro is an optional review channel, not a prerequisite or a required subscription. Use it only when explicitly selected and when it can meet the same full-source and exact-candidate contract. If that channel is unavailable, use the independent Codex path; do not turn a failed or unrun web review into a PASS. This policy applies only to this Taskboard repository, not to shared Skills or other managed projects.
...
- Independent dispatched conversations run their required reviews in parallel when resources permit. Do not serialize independent reviews through the coordinating conversation.
- Wait for the complete review result, regardless of channel. Do not substitute a preliminary response, a fixed elapsed time, or a routine status check for the completed verdict.
- Fix actionable blockers in the same PR. The dispatched execution conversation decides the affected review scope; complex or risky corrections retain independent review, while trivial targeted follow-up edits can use its normal Agent review. Required CI, branch protections, and repository-specific merge authority remain unchanged.
...
- User UI confirmation is a final acceptance gate, not an intermediate development checkpoint. For work that requires it, ask only after the full function is complete, direct verification passes, and the required risk-based review passes. Never ask the user to confirm a partially implemented UI.
- After review approval, visual-only adjustments made from the user's final UI feedback do not require another independent review. The coordinator checks that the delta is limited to the requested visual change, reruns the real path, and can then proceed to merge. If the adjustment changes functional logic or introduces new complex risk, reassess the required review scope and independence.
...
- When work meets the UI confirmation gate, put the complete reviewed function into the Taskboard-launched Codex App and ask the user to confirm the final visual style only after implementation and the required risk-based review are complete.
