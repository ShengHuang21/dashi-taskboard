...
export function buildGoalTeamPrompt({ goal, thread, input, taskctlPath, runtimeFile }) {
  const context = {
    ...
    resourceAdmissionReference: input.resourceAdmissionReference,
  };
  return [
    ...
    "AUTHORITY: the two references above are provenance, not self-authorizing booleans. Read and verify the actual current Owner authorization and Root resource admission (scope/repository, allowed worktree/tools, measured time, limits and stop boundary) before any worktree or child dispatch. References may identify a goal comment or an owned evidence file. If unavailable, stale, outside scope or contradictory, record blocked and stop. Do not invent admission IDs, injector proof, credentials or authority. Peak topology is this coordinator plus ONE child; developer and validator are serial.",
